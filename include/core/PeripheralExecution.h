...
// 预定义的触发事件列表（静态事件）
static const EventDef STATIC_EVENTS[] = {
    // WiFi 事件
...
// 获取静态事件数量
inline size_t getStaticEventCount() {
    size_t count = 0;
    while (STATIC_EVENTS[count].id != nullptr) count++;
    return count;
}

// 根据ID查找静态事件
inline const EventDef* findStaticEvent(const char* id) {
