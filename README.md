...
> Standard 版 OTA 仅 `esp32s3-F8R0`（8MB Flash）支持，`esp32-F4R0`（4MB Flash）受空间限制不含 OTA。
...
无需安装任何工具，打开浏览器即可一键烧录固件体验系统：

👉 **[在线烧录工具](https://fastbee.cn/doc/device/esp32-flasher.html)**
...
| --- | --- |
| 快速开始 | 从烧录到创建第一条规则 |
| 部署与版本验证 | 固件、LittleFS、发布包和真实设备验证 |
| 测试与版本验证 | 静态检查、native、全芯片编译、冒烟和稳定性测试 |
| 版本对比 | Lite、Standard、Full 功能差异 |
| 项目结构 | 目录、模块和构建产物说明 |
| 用户手册 | Web 页面和功能操作说明 |
| 支持模块清单 | 各版本支持的模块、传感器和接入方式 |
| 外设文档 | GPIO、传感器、显示、RFID、Modbus 外设 |
| 外设执行 | 触发器、动作和联动规则 |
| 示例教程 | LED、继电器、传感器、显示屏和 Modbus 示例 |
| 商用授权 | AGPL 协议与商用授权说明 |

## 许可证

本项目采用 [AGPL-3.0](LICENSE) 开源协议，个人免费使用，商用需购买授权。
