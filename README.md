...
> 版本选择的核心依据是 **Flash 容量**和**是否有 PSRAM**。Flash ≥8MB 的环境均支持 OTA 升级（4MB 环境受空间限制不支持）。带 PSRAM 的模组（如 ESP32-WROVER、ESP32-S3-N8R2/N16R8）均可使用 Full 版。预定义的环境不满足需求可以platformio中新增，配置合适flash、psram和需要支持的外设
...
| 3 | **网络配置** | 选择联网方式（WiFi / 以太网 / 4G），填写参数后保存 | 网络配置 |
