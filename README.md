...
### 📡 ESP32 全系列支持 — 多芯片、多外设、多配网

- **三大芯片**：ESP32（双核 240MHz）、ESP32-S3（AI 加速）、ESP32-C3（低成本 RISC-V）
- **智能配网**：WiFi STA / AP / 双模、蓝牙 BLE 配网、mDNS 发现
- **IP 冲突三层检测**：确保设备稳定入网
- **35+ 编译开关**：3 种预设方案（full / minimal / release），按需裁剪
...
<p align="center">
  <img src="./images/device.png" width="600" alt="FastBee 物联网终端"/>
</p>
...
- 首次启动自动进入 **AP 配网模式**，连接热点 `fastbee-ap`
...
| 协议 | 特性 |
|------|------|
| MQTT | 双认证(简单/AES加密)、QoS 0/1/2、TLS、断线指数退避重连、环形缓冲队列 |
| Modbus RTU | 工业级 RS485 总线、主从双模、8 种功能码、16 子设备、5 种设备类型（详见下方） |
...
| 透传模式 | RAW HEX 直转发，可用于非标准协议或调试场景 |
...
项目提供 **6 种芯片编译预设** 和 **3 种功能方案**：

| 预设方案 | 说明 | 适用场景 |
|----------|------|----------|
| `full` | 全功能开启（MQTT + Modbus + CoAP + TCP + HTTP + BLE） | 开发调试、功能评估 |
| `minimal` | 最小功能集（仅 MQTT + Web） | 资源受限场景 |
| `release` | 生产优化（关闭调试日志、启用优化） | 量产部署 |

```ini
# platformio.ini 中切换预设
[env:esp32-full]
board = esp32dev
