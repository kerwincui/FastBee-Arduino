...
| 硬件配置 | PlatformIO 环境 | 版本 | 主要能力 |
| --- | --- | --- | --- |
| ESP32-C3 4MB | `esp32c3` | Lite | WiFi、MQTT、基础外设、执行规则、命令脚本 |
| ESP32-C6 4~8MB | `esp32c6` | Lite | 同上，支持 WiFi 6 |
| ESP32 4MB（无 PSRAM） | `esp32` | Standard | Lite + Modbus、以太网、4G、I2C 传感器、RFID |
| ESP32 ≥8MB + PSRAM | `esp32` | Standard | 同上，余量更充足 |
| ESP32-S3 8MB（无 PSRAM） | `esp32s3` | Standard | 同上，更高性能双核 |
| ESP32-S3 ≥8MB + ≥2MB PSRAM | `esp32s3-full` | Full | Standard + OTA、文件管理、日志、多用户、LoRa、BLE |

> 版本选择的核心依据是 **Flash 容量**和**是否有 PSRAM**。带 PSRAM 的模组（如 ESP32-WROVER、ESP32-S3-N8R2）均可使用 Full 版，需自行调整分区表匹配实际 Flash 大小。
