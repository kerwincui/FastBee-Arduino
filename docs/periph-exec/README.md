...
规则列表用于确认每条规则的启用状态、触发器数量、动作数量和手动执行入口。现场首次上线时建议逐条验证，再打开“启用”。

截图要点：外设执行页面是自动化规则的总入口。阅读截图时先确认规则是否启用，再看触发器类型和动作数量；调试阶段可以用“手动执行”验证动作链，稳定后再交给定时、轮询、事件或平台触发器自动运行。

![外设执行规则数据模型](../images/periph-exec-rule-data-model.svg)

![触发器时序对比](../images/trigger-timing-comparison.svg)

上面两张图适合先看：数据模型说明一条规则由哪些字段组成，时序对比说明什么时候该选择平台、定时、事件或轮询触发。
