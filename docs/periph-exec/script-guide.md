...
> 当前默认 `esp32`、`esp32c3`、`esp32s3` 精简版关闭 `FASTBEE_ENABLE_RULE_SCRIPT`，Web 页面也不发布 RuleScript 管理入口。本文档主要适用于 `esp32s3-full` 完整版或自行开启 RuleScript 的定制构建。精简版的本地联动请优先使用“外设执行”中的定时、事件、传感器和 Modbus 动作。

完整版中的规则脚本页面用于维护脚本列表；轻量命令脚本通常从外设执行动作中进入。

![规则脚本页面](../system/images/rule-script.png)

![外设执行规则列表](../system/images/periph-exec-management.png)

截图要点：脚本页面用于管理可复用逻辑，外设执行页面用于确认脚本被哪条规则调用。排查脚本问题时建议同时保留这两张截图，再配合设备日志里的脚本执行结果定位语法错误、超时或变量状态异常。

![脚本与模板转换管道](../images/script-transform-pipeline.svg)

脚本调试建议先从固定字段模板开始，确认输入数据、规则匹配和输出 payload 都正确后，再逐步加入条件、表达式和外设动作。

![命令脚本执行流程](../images/command-script-execution-flow.svg)

命令脚本更适合短动作序列，例如流水灯、PWM 渐变或简单显示刷新。复杂判断仍建议放在外设执行触发器或 RuleScript 中。
...
![规则脚本模板映射](../images/rule-script-template-map.svg)

调试规则脚本时，先把输入 JSON、占位符和输出模板三者摆在一起核对。若 `${key}` 保持原文，通常说明输入数据里没有同名 key，或数组格式没有按 `id/value` 提取。
...
![命令脚本语法速查](../images/command-script-cheatsheet.svg)

命令脚本适合短小、顺序、可审计的硬件动作。涉及复杂条件、循环判断或长时间等待时，优先把条件放到触发器里，把脚本保留为明确的执行步骤。
...
这些限制的目标是保证脚本不会长期阻塞设备主流程。若必须执行较长过程，建议拆成多条规则，通过事件或平台命令分阶段触发，并在每个阶段写入日志。
...
脚本排障建议先分类：保存失败看语法和长度，规则未触发看触发器和启用状态，变量未替换看输入 JSON 与占位符，动作无效果看目标外设和日志。不要一上来就重写脚本。
