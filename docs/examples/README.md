...
所有示例都可以用同一套 Web 流程落地：在“外设配置”添加硬件对象，在“外设执行”创建采集、显示或控制规则，必要时在“通信协议”配置 MQTT/Modbus。
...
| 页面入口 | 对照截图 | 建议先看 | 配置重点 |
|------|------|------|------|
| 外设配置 | ![外设配置](../system/images/peripheral-management.png) | [01 LED](01-led-first.md)、[06 按键](06-button-control.md)、[19 DHT11](19-dht11-humidity.md) | 新增硬件对象，核对类型、引脚、参数和启用状态。 |
| 外设执行 | ![外设执行](../system/images/periph-exec-management.png) | [02 LED 闪烁](02-led-blink.md)、[10 定时触发](10-timer-interrupt.md)、[33 光敏开灯](33-light-sensor.md) | 创建触发器和动作，先禁用保存，再执行一次验证。 |
| Modbus RTU | ![Modbus RTU](../system/images/protocol-modbus-rtu.png) | [45 Modbus 温湿度](45-modbus-temp-humidity.md)、[47 Modbus 继电器](47-modbus-relay.md)、[48 土壤传感器](48-modbus-soil-sensor.md) | 确认串口、波特率、从站地址、功能码和寄存器映射。 |
| 设备配置 | ![设备配置](../system/images/device-config.png) | [16 NTP 时钟](16-rtc-clock.md)、[17 DS1302 RTC](17-ds1302-rtc.md) | 确认设备时间、NTP、开发环境功能和配置备份。 |
| 文件管理 | ![文件管理](../system/images/file-management.png) | [24 SD 卡](24-sd-card.md)、批量配置迁移场景 | 查看 LittleFS 文件，导入导出配置，保留现场备份。 |

示例覆盖不只按编号排列，也可以按硬件能力矩阵检查是否已经具备“文档、外设类型、Web 配置、规则联动、版本开关”五个环节。
