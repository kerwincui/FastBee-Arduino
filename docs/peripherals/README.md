...
外设页面用于把真实硬件抽象为可被规则引用的对象。新增时先选择类型，再填写引脚和参数；首次保存建议保持禁用，确认接线后再启用。
...
选型时先按硬件信号形态判断外设类型：开关量走 GPIO，模拟量走 ADC，总线设备走 I2C/SPI/UART，RS485 从站走 Modbus 子设备，显示类外设先跑通静态文本。
...
可视化配置后建议在文件管理页保留一份 LittleFS 配置备份，便于批量复制到同型号设备或回滚现场修改。
