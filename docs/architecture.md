...
FastBee-Arduino 定位为 **嵌入式物联网软件系统**，面向设备接入、数据采集、协议上报、远程控制和现场维护。Lite、Standard、Full 三个版本允许功能裁剪，但基础稳定能力必须保持一致：系统可以启动、配置可以恢复、网络异常可以重连、平台连接异常可以恢复、内存不足可以降级保护、故障可以通过日志或错误码定位。

| 能力 | Lite 轻量版 | Standard 标准版 | Full 全功能版 |
|------|-------------|------------------|---------------|
| 启动与自检 | 必须支持 | 必须支持 | 必须支持 |
| WiFi/AP 配网 | 必须支持 | 必须支持 | 必须支持 |
| 网络断线重连 | 必须支持 | 必须支持 | 必须支持 |
| MQTT 数据上报 | 必须支持 | 必须支持 | 必须支持 |
| 配置读写保护 | 必须支持 | 必须支持 | 必须支持 |
| 看门狗与健康监控 | 必须支持 | 必须支持 | 必须支持 |
| 故障码/关键日志 | 基础故障码 | 完整故障码 | 完整故障码与远程诊断 |
| 本地缓存/文件能力 | 精简 | 按资源启用 | 完整启用 |
| OTA | 默认关闭 | 可按资源启用 | 默认启用并必测 |
| 远程配置 | 基础配置 API | 常用配置 API | 完整配置、文件、用户与角色能力 |

### 长期运行设计原则

1. 功能可裁剪，恢复链路不可裁剪：启动、配网、网络重连、配置保护、健康监控和日志必须在三个版本中保留。
2. Full 版本承载更多能力，必须依赖 PSRAM、内存门控和接口降级策略；运行时如果 `psramTotal` 为 0，应视为硬件或构建档位异常。
3. Lite 版本优先保证低资源稳定运行，不默认开启 OTA、文件管理、用户角色、RuleScript 等重功能。
4. Standard 版本用于常规业务部署，应覆盖 Modbus、命令脚本、以太网/4G 等现场常用能力，但仍需按实际 Flash/RAM 预算裁剪。
5. 所有版本发布前都必须通过构建、冒烟和长稳验证；Full 版本还必须覆盖 OTA、远程配置、日志/文件和权限相关接口。

### 启动与异常恢复链路

```mermaid
flowchart TD
    BOOT[上电启动] --> FS[挂载 LittleFS / NVS]
    FS --> CFG{配置有效?}
    CFG -- 是 --> INIT[初始化外设/网络/协议]
    CFG -- 否 --> FALLBACK[加载默认配置并记录故障码]
    FALLBACK --> INIT
    INIT --> NET{网络可用?}
    NET -- 是 --> PROTO[启动 MQTT/Modbus/API]
    NET -- 否 --> AP[保持 AP 配网与重连任务]
    AP --> RETRY[指数退避重连]
    RETRY --> NET
    PROTO --> RUN[进入主循环和健康监控]
    RUN --> HEALTH{内存/连接/任务正常?}
    HEALTH -- 是 --> RUN
    HEALTH -- 否 --> DEG[降级服务/关闭重接口/记录错误]
    DEG --> RUN
```
