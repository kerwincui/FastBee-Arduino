...
它基于标准 Modbus RTU 功能码实现，并针对**国产通用型 Modbus RTU 继电器模块**（闪开型）进行了默认适配，支持其原生指令集。主要功能：

- 继电器模块的线圈开关控制（原生翻转指令 0x5500）
- 闪开延时控制（硬件定时，FC 0x05 地址 0x0200+通道）
- 批量开/关/翻转操作（原生全翻转指令 0x5A00）
- 离散输入（DI）状态读取
- 从站设备地址管理（广播读取/写入）
- 从站设备波特率配置（FC 0xB0 专有指令）
- 通信调试和设备排查
...
|------|------|------|------|
| slaveAddress | int | 是 | 从站地址 (1-247) |
| baudRate | int | 是 | 波特率 (1200/2400/4800/9600/19200/115200) |

**说明**：使用 FC 0xB0 专有指令，帧格式 `[SlaveAddr, 0xB0, 0x00, 0x00, BaudCode, 0x00, CRC]`。

**成功响应**：
```json
{
  "success": true,
  "data": { "baudRate": 9600, "baudCode": 3 }
}
```
