...
- `MAX_ACTIVE_TASKS = 12`：同时运行的规则数上限
- 系统内存门控（MEMGUARD）：低内存时自动降级
...
> **注意**：内存紧张时系统会自动进入 `degraded` 模式，Web API 列表分页会临时缩减为每页 5 条，确保页面可访问。外设配置达到硬性上限后会返回错误，执行规则超过推荐值时会记录警告但不阻止创建。
